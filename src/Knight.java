...
//    private int[] distTo; 
    ...
    public static int knight(int N, int M, int x1, int y1, int x2, int y2) {
        ...
        Point start = Knight.Point(x1, y1);
        Point end = Knight.Point(x2, y2);
        
        Queue<Point> queue = new LinkedList<Point>();
       
        for(Point p : knight.adj(i, j)){
            p.visited = true;
    ...
    private static Point Point(int x1, int y1) {
		return Knight.Point(x1, y1);
	}
    ...
    public static void main(String[] args) {
    	System.out.println(knight(9, 9, 2, 1, 6, 5));
    }
}